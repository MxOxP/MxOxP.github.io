...
function TiraDadO(turno) {
    ...
    function Preguntas(dado, turno) {
        ...
        RandomPreguntas = Math.floor(Math.random() * preguntas.length);
        var OpcionesRes = respuestas[RandomPreguntas];
        var txtRespuestas = ""
        ...
        bootbox.alert({
            ...
            //Quitar boton X y centrar Vertical Popup
            closeButton: false,
            centerVertical: true,
            //----------

            message: preguntas[RandomPreguntas] + txtRespuestas,
            callback: function () {
                var respuesta = $("input[type=radio]:checked").val();
                Comprobar(respuesta, RandomPreguntas, dado, turno);
            }
        });
    ...
    function Comprobar(respuesta, RandomPreguntas, dado, turno) {
        var respuestaCorrecta = [1, 1, 1, 0, 1, 0, 0, 2, 1, 0, 2, 0, 0, 0, 1, 1, 0, 1, 1, 1, 0, 2, 2, 1, 2];
        if (respuesta == respuestaCorrecta[RandomPreguntas]) {
            bootbox.alert("Correcto avanzaste");
            Avanzar(dado, turno);
            


            //   Avanzar ficha si es correcto
            var JSON_POSICION;
            readTextFile("../slides/PosicionFicha.json", function (text) { //cambio 1
            ...
            bootbox.alert("Incorrecto no avanzas");
